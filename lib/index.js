...
exports.default = transformPackage;
...
function transformPackage(packageTransformDir, packagePath) {
  return readPaths(packageTransformDir).then(function (packageTransformPaths) {
    ...
    return readJSON('package.config.json').then(function (config) {
      return Promise.all(imports.map(function (importPath) {
        ...
        return readJSON(packagePath).then(function (packageJSON) {
          return Promise.all(packageTransformPaths.filter(function (x) {
      ...
      throw err;
    }); //new Error('No package.config.json found. Add one to your project root as specified at https://transform-package.js.org'))
  });
}
...
var readJSON = function readJSON(jsonPath) {
  return new Promise(function (resolve, reject) {
